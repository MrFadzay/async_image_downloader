...
class TestImageHashing:
    ...
    def test_hash_difference(self, temp_dir):
        ...
        # Create different images with more complex patterns
        image_path1 = temp_dir / "pattern1_image.jpg"
        image_path2 = temp_dir / "pattern2_image.jpg"

        # Create image with checkerboard pattern
        pattern1 = Image.new('RGB', (100, 100), color='white')
                    ...
                    for i in range(10):
                        for j in range(10):
                            if x + i < 100 and y + j < 100:
                                pattern1.putpixel((x + i, y + j), (0, 0, 0))

        # Create image with diagonal stripes
        pattern2 = Image.new('RGB', (100, 100), color='white')
        for x in range(100):
            for y in range(100):
                if (x + y) % 20 < 10:
                    pattern2.putpixel((x, y), (255, 0, 0))

        pattern1.save(image_path1, format='JPEG')
        pattern2.save(image_path2, format='JPEG')
...
class TestGetFileHashes:
    ...
    async def test_get_file_hashes_different_images(self, temp_dir):
        ...
        # Create different images with distinct patterns
        red_image_path = temp_dir / "red.jpg"
        green_image_path = temp_dir / "green.jpg"
        blue_image_path = temp_dir / "blue.jpg"

        # Create images with different patterns to ensure different hashes
        # Red image with horizontal stripes
        red_img = Image.new('RGB', (100, 100), color='white')
        for y in range(0, 100, 10):
            for x in range(100):
                red_img.putpixel((x, y), (255, 0, 0))
        red_img.save(red_image_path, format='JPEG')

        # Green image with vertical stripes
        green_img = Image.new('RGB', (100, 100), color='white')
        for x in range(0, 100, 10):
            for y in range(100):
                green_img.putpixel((x, y), (0, 255, 0))
        green_img.save(green_image_path, format='JPEG')

        # Blue image with diagonal pattern
        blue_img = Image.new('RGB', (100, 100), color='white')
        for i in range(100):
            for j in range(100):
                if (i + j) % 20 < 10:
                    blue_img.putpixel((i, j), (0, 0, 255))
        blue_img.save(blue_image_path, format='JPEG')
    ...
    async def test_get_file_hashes_ignores_non_images(self, temp_dir):
        ...
        Image.new('RGB', (100, 100), color='yellow').save(
...
class TestGetImageFiles:
    ...
    async def test_get_image_files_ignores_hidden_files(self, temp_dir):
        ...
        Image.new('RGB', (50, 50), color='red').save(
            visible_image, format='JPEG')
        Image.new('RGB', (50, 50), color='blue').save(
            hidden_image, format='JPEG')
...
class TestProcessAndSaveImageSync:
    ...
    def test_process_and_save_image_sync_valid_jpeg(self, temp_dir, sample_image_data):
        ...
        process_and_save_image_sync(
            sample_image_data, output_path, "image/jpeg")
    ...
    def test_process_and_save_image_sync_large_file(self, temp_dir, large_image_data):
        ...
        with pytest.raises(ValueError, match="Размер изображения превышает ограничения"):
            process_and_save_image_sync(
                large_image_data, output_path, "image/jpeg")
    ...
    def test_process_and_save_image_sync_invalid_data(self, temp_dir):
        ...
        output_path = temp_dir / "output.jpg"
        # Create invalid data larger than MIN_FILE_SIZE (100 bytes)
        invalid_data = b"This is not valid image data, but it's long enough to pass size validation. " * 3
...
class TestSimilarityDetection:
    ...
    def test_similarity_threshold_logic(self):
        ...
        test_cases = [
            ...
            # Similar: 2/3 matches >= threshold
            (("a", "b", "c"), ("a", "b", "d"), True),
            # Different: 1/3 matches < threshold
            (("a", "b", "c"), ("a", "x", "y"), False),
            # Completely different: 0/3 matches
            (("a", "b", "c"), ("x", "y", "z"), False),
        ]
