...
*   **Обработка дубликатов:**
    ...
    *   **Уникализация дубликатов:** Модифицирует только дубликаты изображений (изменяя яркость, контраст, обрезку или добавляя шум), чтобы сделать их уникальными, сохраняя при этом визуальное сходство.
    *   **Уникализация всех изображений:** Применяет модификации ко всем изображениям в директории, независимо от того, являются ли они дубликатами.
*   **Два режима работы:**
...
*   **Из файла с URL:**
    ...
    **С задержкой между запросами (рекомендуется для Авито и других защищенных сайтов):**
    ```bash
    python async_image_downloader.py download --file url.txt --delay 2
    ```
...
*   **По списку URL:**
    ...
    **С задержкой между запросами:**
    ```bash
    python async_image_downloader.py download --urls https://example.com/img1.jpg https://example.com/img2.png --dest my_downloads --delay 1.5
    ```
...
```bash
python async_image_downloader.py uniquify-all images/my_downloads
```
*(Замените `images/my_downloads` на путь к вашей директории)*
