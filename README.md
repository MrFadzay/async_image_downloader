...
- Автоматическая установка браузера при первом использовании
...
*   **Два режима скачивания:**
    *   **Быстрый режим (по умолчанию):** Использует aiohttp для максимальной скорости (~1 сек/изображение)
    *   **Браузерный режим:** Использует настоящий браузер для обхода защиты сайтов (~4-5 сек/изображение)
*   **Асинхронное скачивание:** Эффективно скачивает изображения из списка URL или из файла, содержащего URL.
*   **Защита от блокировки:** Настраиваемая задержка между запросами для предотвращения блокировки со стороны сайтов.
*   **Автоустановка браузера:** При первом использовании браузерного режима программа автоматически установит необходимый браузер.
*   **Обработка дубликатов:**
    *   **Поиск и переименование:** Находит дубликаты на основе трех алгоритмов хеширования
    *   **Уникализация:** Модифицирует дубликаты (яркость, контраст, обрезка, шум) для создания уникальных файлов
*   **Два режима работы:**
...
2.  **Установите зависимости:**
    ...
    **Полная версия (с браузерным режимом):**
    ```bash
    pip install -r requirements.txt
    ```
    
    **Минимальная версия (только быстрый режим):**
    ```bash
    ...
    или вручную:
    ```bash
    pip install aiohttp aiofiles Pillow imagehash questionary playwright
    ```
...
*   **Из файла с URL:**
    ...
    ```bash
    python main.py download --file url.txt
    ```
    
    **С задержкой между запросами (рекомендуется для Авито и других защищенных сайтов):**
    ```bash
    python main.py download --file url.txt --delay 2
    ```
    
    **С браузерным режимом (для сайтов с защитой от ботов):**
    ```bash
    python main.py download --file url.txt --browser-mode
    ```
...
*   **По списку URL:**
    ...
    ```bash
    python main.py download --urls https://example.com/img1.jpg https://example.com/img2.png --dest my_downloads
    ```
    
    **С задержкой между запросами:**
    ```bash
    python main.py download --urls https://example.com/img1.jpg https://example.com/img2.png --dest my_downloads --delay 1.5
    ```
    
    **С браузерным режимом (для защищенных сайтов):**
    ```bash
    python main.py download --urls https://avito.ru/img1.jpg https://avito.ru/img2.jpg --dest avito_images --browser-mode --delay 3
    ```
