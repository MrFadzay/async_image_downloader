...
def validate_file_size(file_size: int, max_size: int = MAX_DOWNLOAD_SIZE) -> bool:
    ...
    if file_size > max_size:
        logger.warning(
            f"Файл слишком большой: {file_size} байт " f"(максимум {max_size / (1024*1024):.1f} MB)"
        )
        return False
...
def validate_mime_type(content_type: str) -> bool:
    ...
    """
    # Проверяем, что content_type не None и не пустая строка
    if not content_type:
        logger.warning("Пустой или отсутствующий MIME-тип")
        return False
