...
class DownloadSessionManager:
    ...
    def _setup_signal_handlers(self) -> None:
        ...
        def signal_handler(sig, frame):
            logger.info("Получен сигнал прерывания. Приостанавливаем загрузку...")
            self.pause()
    ...
    async def create_session(
        ...
        if target_dir is None:
            from utils.config import DEFAULT_DOWNLOAD_DIR_NAME
    ...
    async def save_session(self) -> None:
        ...
        try:
            ...
            async with aiofiles.open(self.session_file, 'w', encoding='utf-8') as f:
                await f.write(json.dumps(asdict(self.current_session), 
                                       ensure_ascii=False, indent=2))
            logger.debug(f"Сессия сохранена: {self.current_session.session_id}")
        except Exception as e:
            logger.error(f"Ошибка при сохранении сессии: {e}")
    
    async def load_session(self, session_id: Optional[str] = None) -> Optional[DownloadSessionState]:
        """
    ...
    def get_remaining_urls(self) -> List[str]:
        ...
        processed_urls = set(self.current_session.completed_urls + self.current_session.failed_urls)
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Статистика сессии
        """
        if not self.current_session:
            return {}
        ...
        return {
            ...
            'completed': len(self.current_session.completed_urls),
            'failed': len(self.current_session.failed_urls),
            'remaining': len(self.get_remaining_urls()),
            'progress_percent': (len(self.current_session.completed_urls) / 
                               self.current_session.total_urls * 100) if self.current_session.total_urls > 0 else 0,
