...
class ConfigManager:
                ...
                if section_name in data:
                    ...
                    for key, value in section_data.items():
                        if key in current_dict:
                            ...
                            logger.warning(f"Неизвестный параметр конфигурации: {section_name}.{key}")
                    
                    # Создаем новый объект секции
                    setattr(config, section_name, section_class(**current_dict))
    ...
    def validate_config(self) -> bool:
        ...
        if self.config.download.max_concurrent_downloads <= 0:
            errors.append("download.max_concurrent_downloads должно быть больше 0")
        ...
        if self.config.validation.max_download_size_mb <= 0:
            errors.append("validation.max_download_size_mb должно быть больше 0")
        if self.config.validation.min_file_size < 0:
            errors.append("validation.min_file_size не может быть отрицательным")
...
def load_or_create_config(config_dir: Optional[Path] = None) -> AppConfig:
    ...
    if not loaded:
        ...
        try:
            ...
            logger.info(f"Создан образец конфигурации: {sample_file}")
            logger.info("Отредактируйте файл и переименуйте в 'config.yaml' для использования")
        except Exception as e:
