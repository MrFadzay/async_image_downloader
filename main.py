...
from utils.config_manager import load_or_create_config, ConfigManager
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    """
    parser = argparse.ArgumentParser(description="Async Image Downloader and Processor.")
    ...
    parser.add_argument(
        "--list-profiles", action="store_true", help="Показать список доступных профилей"
    )

    subparsers = parser.add_subparsers(dest="command", required=False)

    # --- Команда download ---
    p_download = subparsers.add_parser("download", help="Скачать изображения по списку URL.")
    p_download.add_argument(
        "urls",
        nargs="+",
        help=("Список URL для скачивания, разделенных пробелами."),
    )
    p_download.add_argument(
        "-s",
        "--start-index",
    ...
    p_uniq = subparsers.add_parser(
        "uniquify",
        help=("Find and modify duplicate images to make them unique."),
    )
    ...
    p_uniq_all = subparsers.add_parser("uniquify-all", help="Uniquify all images in directory.")
    p_uniq_all.add_argument("directory", type=Path, help="Directory to process.")
    ...
    if args.command == "download":
        if hasattr(args, "enable_pause_resume") and args.enable_pause_resume:
            ...
            return run_download_session(
                urls=args.urls, start_index=args.start_index, retries=args.retries
            )
...
def main() -> None:
    ...
    if args.save_profile:
        ...
        success = ConfigProfiles.save_profile_as_config(args.save_profile, output_file)
        if success:
            print(f"✅ Профиль '{args.save_profile}' сохранен в {output_file}")
            print("💡 Переименуйте в 'config.yaml' для использования по умолчанию")
        else:
    ...
    if args.profile:
        ...
        try:
            ...
            logger.info(f"Используется профиль '{args.profile}' " f"(версия {config.version})")
